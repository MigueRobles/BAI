...
\subsection{Primary Care Chatbot}
\label{sec:primary}
But we should not only use the tools that today's technologies provide us to improve our social life, we should also make use of them to improve our quality of life in the health aspect. In \cite{ni2017mandy} they present us with a robot that is able to talk to a person in distress and give him the necessary instructions to carry out the first aid maneuvers and even to save a life. This chatbot is used through a mobile application and is able to receive symptoms from a patient and give a diagnosis, although there is still much to improve The system combines data-driven natural language processing capability with knowledge-driven diagnostic capability. 
